...
struct c64_common
{
    ...
    static inline std::uint_fast64_t expand32_8(std::uint_fast32_t a)
    {
        // 0000abcd -> 0a0b0c0d
        typedef std::uint_fast64_t v;
        return (a&0xFFU)
            | ((a&0xFF00U)<<8)    // base: 8+8 = 16
            | ((v)(a&0xFF0000U)<<16) // base: 16+16 = 32
            | ((v)(a&0xFF000000UL)<<24); // base: 24+24 = 48
    }
    static inline std::uint_fast64_t expand32_16(std::uint_fast32_t a)
    {
        // 0000abcd -> 00ab00cd
        typedef std::uint_fast64_t v;
        return (a&0xFFFFU)
...
struct c64_MMX: public c64_common
{
    ...
    inline c64_MMX(const std::uint64_t& v) : value( *(const __m64*)& v) { }
    ...
    operator bool() const
    {
        union m64union { struct { std::uint_least32_t a, b; }; __m64 c; };
    ...
    c64 operator~ () const {
        static const std::uint_least64_t negpat = ~(std::uint_least64_t)0;
    ...
    c64 operator- (const c64& b) const
    {
        ...
        #else
        return (const std::uint64_t&)value - (const std::uint64_t&)b.value;
        #endif
    }
    c64 operator+ (const c64& b) const
    {
    ...
    c64 operator& (const std::uint64_t& v) { return c64(_mm_and_si64(value, *(const __m64*)& v)); }
...
struct c64_nonMMX: public c64_common
{
    ...
    typedef std::uint_least64_t valuetype;
    valuetype value;

    inline c64_nonMMX() : value() { }
    inline c64_nonMMX(std::uint64_t v) : value(v) { }
    ...
    void Init16(short a,short b,short c, short d)
        { std::uint_fast64_t aa = (unsigned short)a,
                        bb = (unsigned short)b,
                        cc = (unsigned short)c,
                        dd = (unsigned short)d;
    ...
    {
        value = ((std::uint_fast64_t)(a | (b << 8) | (c << 16) | (d << 24)))
              | (((std::uint_fast64_t)e) << 32)
              | (((std::uint_fast64_t)f) << 40)
              | (((std::uint_fast64_t)g) << 48)
              | (((std::uint_fast64_t)h) << 56);
    }

    void Get(const unsigned char* p)      { value = *(const std::uint_least64_t*)p; }
    void Put(      unsigned char* p)const { *(std::uint_least64_t*)p =  value; }
    ...
    c64 operator& (std::uint_fast64_t b) const { return value & b; }
    ...
    c64 unpacklbw(const c64& p) const
    {
        ...
        return (std::uint_least64_t)_m_punpcklbw(*(const __m64*)&p.value, *(const __m64*)&value);
    #else
        std::uint_fast64_t a=value, b=p.value;
    ...
    {
    #if defined(__MMX__) && !defined(__ICC)
        return (std::uint_least64_t)_m_punpckhbw(*(const __m64*)&p.value, *(const __m64*)&value);
    #else
        std::uint_fast64_t a=value, b=p.value;
    ...
    {
    #if defined(__MMX__) && !defined(__ICC)
        return (std::uint_least64_t)_m_punpcklwd(*(const __m64*)&p.value, *(const __m64*)&value);
    #else
        std::uint_fast64_t a=value, b=p.value;
    ...
    {
    #if defined(__MMX__) && !defined(__ICC)
        return (std::uint_least64_t)_m_punpckhwd(*(const __m64*)&p.value, *(const __m64*)&value);
    #else
        std::uint_fast64_t a=value, b=p.value;
    ...
    {
    #if defined(__MMX__) && !defined(__ICC)
        return (std::uint_least64_t)_m_punpckldq(*(const __m64*)&p.value, *(const __m64*)&value);
    #else
